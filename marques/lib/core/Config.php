...
class Config {
     ...
     */
    public function __construct(?Path $appPath = null) {
        // Falls keine Path-Instanz übergeben wurde, erstelle eine neue:
        if ($appPath === null) {
            $appPath = new Path();
        }
    ...
    public function load(string $name, bool $forceReload = false): ?array {
        ...
        if (!file_exists($filePath)) {
            error_log("Config: Konfigurationsdatei nicht gefunden: " . $filePath);
            return null;
        }
        if (!is_readable($filePath)) {
            error_log("Config: Konfigurationsdatei nicht lesbar: " . $filePath);
    ...
    public function save(string $name, array $data): bool {
        ...
        if (!is_dir($dir)) {
            if (!@mkdir($dir, 0755, true)) {
                error_log("Config: Konnte Verzeichnis nicht erstellen: " . $dir);
                return false;
            }
        }
        ...
        if (file_put_contents($tempFile, $content) === false) {
            error_log("Config: Fehler beim Schreiben der temporären Datei: " . $tempFile);
        ...
        if (!@rename($tempFile, $filePath)) {
            error_log("Config: Fehler beim Umbenennen der temporären Datei zu: " . $filePath);
            @unlink($tempFile);
            return false;
        }
     ...
     */
    public function get(string $key, string $configName = 'system', $default = null) {
        $data = $this->load($configName);
        if ($data === null) {
            return $default;
        }
        if (strpos($key, '.') !== false) {
    ...
    private function ensureHtaccessExists(): void {
        // Hier nutzen wir den Path-Zugriff (optional: könntest du auch preparePath() einsetzen)
        $configDir = (new Path())->preparePath('config');
