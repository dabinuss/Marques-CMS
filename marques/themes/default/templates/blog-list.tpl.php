    ...
    <div class="blog-main">
        ...
        <?php else: ?>
            <?php foreach ($posts as $post): ?>
                <article class="blog-post-summary">
                    <header>
                        ...
                        <div class="post-meta">
                            <span class="post-date"><?= \Marques\Core\Helper::formatDate($post['date'], 'd.m.Y'); ?></span>
            ...
            <div class="blog-pagination">
                <?php if ($tpl->page > 1): ?>
                    <a href="<?= \Marques\Core\Helper::getSiteUrl('blog?page=' . ($tpl->page - 1) . (!empty($tpl->category) ? '&category=' . urlencode($tpl->category) : '') . (!empty($tpl->tag) ? '&tag=' . urlencode($tpl->tag) : '')); ?>" class="pagination-prev marques-button marques-button--outline">
                        « Vorherige
                    </a>
                <?php endif; ?>

                <?php if (count($posts) === $tpl->perPage): ?>
                    <a href="<?= \Marques\Core\Helper::getSiteUrl('blog?page=' . ($tpl->page + 1) . (!empty($tpl->category) ? '&category=' . urlencode($tpl->category) : '') . (!empty($tpl->tag) ? '&tag=' . urlencode($tpl->tag) : '')); ?>" class="pagination-next marques-button marques-button--outline">
                        Nächste »
                    </a>
                <?php endif; ?>
            </div>
