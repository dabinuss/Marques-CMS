    ...
    <div class="blog-main">
        ...
        <?php else: ?>
            ...
            <div class="blog-pagination">
                <?php if ($page > 1): ?>
                    <a href="<?php echo marces_site_url('blog?page=' . ($page - 1) . (!empty($category) ? '&category=' . urlencode($category) : '') . (!empty($tag) ? '&tag=' . urlencode($tag) : '')); ?>" class="pagination-prev marces-button marces-button--outline">
                        &laquo; Vorherige
                    </a>
                <?php endif; ?>
                
                <?php if (count($posts) === $perPage): ?>
                    <a href="<?php echo marces_site_url('blog?page=' . ($page + 1) . (!empty($category) ? '&category=' . urlencode($category) : '') . (!empty($tag) ? '&tag=' . urlencode($tag) : '')); ?>" class="pagination-next marces-button marces-button--outline">
                        Nächste &raquo;
                    </a>
                <?php endif; ?>
            </div>
